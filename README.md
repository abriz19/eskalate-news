...
Copy `.env.example` to `backend/.env` and set values. Example:
...
This starts PostgreSQL on `localhost:5432` with user `eskalate`, password `eskalate_secret`, database `eskalate_news` (see `backend/docker-compose.yml`).

### Configure environment

From the repo root (`eskalate-news/`):

```bash
cp .env.example backend/.env
```

Edit `backend/.env` if you changed DB credentials or port in `backend/docker-compose.yml`.
...
The API is available at `http://localhost:5000` (or the port set in `PORT`).
...
Ensure `JWT_SECRET` is set in `backend/.env` so auth and RBAC tests can verify tokens.
...
To avoid duplicate or excessive read logs when the same user (or guest) hits the same article repeatedly, the API uses an **in-memory cooldown**: at most one `ReadLog` per `(articleId, readerId)` per 10 seconds. Old entries are pruned periodically to limit memory use. Unit tests cover this behavior.
...
Other backend files (`package.json`, `prisma/`, `docker-compose.yml`, etc.) live under `backend/` as well.
...
| Command              | Description                    |
|----------------------|--------------------------------|
| `npm run dev`        | Start dev server (from `backend/`) |
| `npm run build`      | Compile TypeScript to `dist/` |
| `npm test`           | Run Jest unit tests (from `backend/`) |
